...
namespace lray
{
    ...
    //-----------------------------------------------------------
    // Determine intersection of ray and plane
    ...
    //-----------------------------------------------------------
    // Determine intersection of ray and sphere
...
#define LRAY_TEST_RAY_TRIAGNLE_HELPER_BACK(ret_fail) \
    ...
    // Determine intersection of ray and triangle
    bool testRayTriangleFront(f32& t, f32& v, f32& w, const Ray& ray, const Vector3& v0, const Vector3& v1, const Vector3& v2)
    {
        ...
        if(F32_EPSILON < discr){
            //Front
            LRAY_TEST_RAY_TRIAGNLE_HELPER_FRONT(false)
    ...
    bool testRayTriangleBack(f32& t, f32& v, f32& w, const Ray& ray, const Vector3& v0, const Vector3& v1, const Vector3& v2)
    {
        ...
        if(discr < -F32_EPSILON){
            //Back
            LRAY_TEST_RAY_TRIAGNLE_HELPER_BACK(false)
        }else{
            return false;
        }
    ...
    Result testRayTriangleBoth(f32& u, f32& v, f32& w, const Vector3& startPoint, const Vector3& endPoint, const Vector3& v0, const Vector3& v1, const Vector3& v2)
    {
        ...
        u = dot(d1, m);
        v = -dot(d0, m);
        if(!lray::isSameSign(u,v)){
            return Result_Fail;
        }

        m = cross(d, d1);
        w = dot(d0, m);
        if(!lray::isSameSign(u,w)){
            return Result_Fail;
        }
        ...
        const lm128 vz[3])
    {
        ...
        //Determine on front
        lm128 disc0 = _mm_cmplt_ps(_mm_set1_ps(F32_EPSILON), discr);
        ...
        const lm128 vz[3])
    {
        ...
        lm128 discr = dot4vec(cx, cy, cz, dx10, dy10, dz10);
        //Determine on back
        lm128 disc0 = _mm_cmplt_ps(discr, _mm_set1_ps(-F32_EPSILON));
        ...
        const lm128 vz[3])
    {
        ...
        //Determine on front
        lm128 f_disc0 = _mm_cmplt_ps(_mm_set1_ps(F32_EPSILON), discr);
        ...
        //Determine on back
        lm128 b_disc0 = _mm_cmplt_ps(discr, _mm_set1_ps(-F32_EPSILON));
    ...
    bool testRayRectangle(f32& t, const Ray& ray, const Vector3& p0, const Vector3& p1, const Vector3& p2, const Vector3& p3)
    {
        ...
        f32 tv,tw;
        if(v>=0.0f){
            return testRayTriangleFront(t, tv, tw, ray, p0, p1, p2);
        }else{
            return testRayTriangleFront(t, tv, tw, ray, p3, p0, p2);
        }
    }

    //-----------------------------------------------------------
    // Determine intersection of ray and AABB
    bool testRayAABB(f32& tmin, f32& tmax, const Ray& ray, const Vector3& bmin, const Vector3& bmax)
    {
        ...
        for(s32 i=0; i<3; ++i){
            if(absolute(ray.direction_[i])<F32_EPSILON){
    ...
    bool testRayAABB(const Ray& ray, const Vector3& bmin, const Vector3& bmax)
    {
        ...
        //
        f32 adx = absolute(d.x_);
        if(absolute(m.x_) > e.x_ + adx) return false;

        f32 ady = absolute(d.y_);
        if(absolute(m.y_) > e.y_ + ady) return false;

        f32 adz = absolute(d.z_);
        if(absolute(m.z_) > e.z_ + adz) return false;

        adx += F32_EPSILON;
        ady += F32_EPSILON;
        adz += F32_EPSILON;

        if(absolute(m.y_ * d.z_ - m.z_ * d.y_) > (e.y_ * adz + e.z_ * ady)) return false;
        if(absolute(m.z_ * d.x_ - m.x_ * d.z_) > (e.x_ * adz + e.z_ * adx)) return false;
        if(absolute(m.x_ * d.y_ - m.y_ * d.x_) > (e.x_ * ady + e.y_ * adx)) return false;
