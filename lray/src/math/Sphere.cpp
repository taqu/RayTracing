...
namespace lray
{
    ...
    void Sphere::combine(const Sphere& s0, const Sphere& s1)
    {
        ...
        f32 distance = lray::sqrt(dv.x_*dv.x_ + dv.y_*dv.y_ + dv.z_*dv.z_);
        ...
        if(distance<=(d+F32_EPSILON)){
            ...
            *this = lray::lerp(s0, s1, d/distance);
            r_ = r;
        }
    }
    ...
    f32 Sphere::distance(const Sphere& rhs) const
    {
        ...
        return ret;
#else
        Vector4 r = (Vector4)*this - (Vector4)rhs;
        r.w_ = 0.0f;
        return r.length();
#endif
    }
    ...
    {
        lm128 m0 = _mm_loadu_ps(&s0.x_);
        lm128 m1 = _mm_loadu_ps(&s1.x_);
        lm128 d = _mm_sub_ps(m1, m0);
        d = _mm_mul_ps(d, _mm_set1_ps(t));
        d = _mm_add_ps(d, m0);
        Sphere s;
        _mm_storeu_ps(&s.x_, d);
        return s;
    }
}
