...
{
#ifdef LRAY_USE_SSE 
    inline lm128 load(const Quaternion& q)
    {
        return _mm_loadu_ps(&q.w_);
    }

    inline void store(Quaternion& q, lm128& r)
    {
        _mm_storeu_ps(&q.w_, r);
    }

    inline Quaternion store(lm128& r)
    {
        Quaternion q;
        _mm_storeu_ps(&q.w_, r);
        return q;
    ...
    {
        LASSERT(!eye.equals(at));
        lookAt( Vector4(normalize(at-eye)) );
    }
    ...
    void Quaternion::lookAt(const Vector4& dir)
    {
        ...
        f32 radian = lray::acos(d);
        Vector4 axis(normalize(cross3(Vector4::Forward, dir)));
        setRotateAxis(axis.x_, axis.y_, axis.z_, radian);
    }
    ...
    Quaternion Quaternion::operator-() const
    {
        ...
        *((u32*)&f) = 0x80000000U;
        ...
        return ret;
#else
        return Quaternion::construct(-w_, -x_, -y_, -z_);
#endif
    }
    ...
    void Quaternion::getRotationAxis(Vector3& axis) const
    {
        ...
        f32 oneOverTheta = 1.0f / lray::sqrt(thetaOver2Sqr);
    ...
    void Quaternion::getEulerAngles(f32& x, f32& y, f32& z)
    {
        ...
        if(0.9999f<tmp){
            ...
            x = -PI_2 * r31/tmp;
            y = lray::atan2(-r12, -r31*r13);
            z = 0.0f;
            return;
        }

        x = lray::asin(-r31);
        y = lray::atan2(r21, r11);
        z = lray::atan2(r32, r33);
    }
    ...
    {
        f32 over2 = radian * 0.5f;
        return {lray::cosf(over2), lray::sinf(over2), 0.0f, 0.0f};
    }
    ...
    Quaternion Quaternion::rotateY(f32 radian)
    {
        ...
        return {lray::cosf(over2), 0.0f, lray::sinf(over2), 0.0f};
    }
    ...
    Quaternion Quaternion::rotateZ(f32 radian)
    {
        ...
        return {lray::cosf(over2), 0.0f, 0.0f, lray::sinf(over2)};
    }
    ...
    Quaternion Quaternion::rotateAxis(f32 x, f32 y, f32 z, f32 radian)
    {
        ...
        return {lray::cosf(over2), x*sinOver2, y*sinOver2, z*sinOver2};
    }

    Quaternion invert(const Quaternion& q)
    {
        LASSERT(lray::isEqual(q.lengthSqr(), 0.0f) == false);
...
#else
        f32 mag = q.lengthSqr();
        if(lray::isEqual(mag, 0.0f)){
            return Quaternion::identity();
        }
        f32 magOver = 1.0f / lray::sqrt(mag);
        f32 w = q.w_ * magOver;
        f32 x = q.x_ * magOver;
        f32 y = q.y_ * magOver;
        f32 z = q.z_ * magOver;
        return Quaternion::construct(x,y,z,w);
#endif
    }
    ...
    Quaternion normalize(const Quaternion& q, f32 squaredLength)
    {
        ...
        return store(r0);
#else
        f32 magOver = 1.0f / lray::sqrt(squaredLength);
        f32 w = q.w_ * magOver;
        f32 x = q.x_ * magOver;
        f32 y = q.y_ * magOver;
        f32 z = q.z_ * magOver;
    ...
    Quaternion exp(const Quaternion& q, f32 exponent)
    {
        ...
        f32 old = lray::acos(q.w_);

        f32 theta = old * exponent;
        f32 t = lray::sinf(theta)/lray::sinf(old);
        return {lray::cosf(theta), q.x_*t, q.y_*t, q.z_*t};
    }
    ...
    Quaternion mul(const Quaternion& q0, const Quaternion& q1)
    {
        ...
        f32 z = q0.w_ * q1.z_ - q0.x_ * q1.y_ + q0.y_ * q1.x_ + q0.z_ * q1.w_;
#endif
        return Quaternion::construct(w, x, y, z);
#endif
    }
    ...
    {
#ifdef LRAY_USE_SSE 
        lm128 r0 = _mm_set1_ps(x);
        lm128 r1 = load(q);
        r0 = _mm_mul_ps(r0, r1);
        return store(r0);

#else
        return Quaternion::construct(x*q.w_, x*q.x_, x*q.y_, x*q.z_);
#endif
    }
    ...
    Quaternion rotateToward(const Vector4& from, const Vector4& to)
    {
        ...
        axis.w_ = 0.0f;
        return Quaternion::rotateAxis(Vector4(normalize(axis)), lray::acos(cosine));
    }
    ...
    Quaternion slerp(const Quaternion& q0, const Quaternion& q1, f32 t)
    {
        ...
        if(d > 0.99f){
            ...
            f32 sinOmega = lray::sqrt(1.0f - d * d);

            f32 omega = lray::atan2(sinOmega, d);

            f32 s = lray::sinf((1.0f-t) * omega);
            k0 = _mm_set1_ps(s);
        ...
        if(cosOmega > 0.99f){
            ...
            f32 sinOmega = lray::sqrt(1.0f - cosOmega * cosOmega);

            f32 omega = lray::atan2(sinOmega, cosOmega);

            f32 oneOverSinOmega = 1.0f / sinOmega;

            k0 = lray::sinf((1.0f-t) * omega) * oneOverSinOmega;
            k1 = lray::sinf(t * omega) * oneOverSinOmega;
        }
...
namespace
{
    Quaternion slerpNoCheck(const Quaternion& q0, const Quaternion& q1, f32 t)
    {
        ...
        if(cosOmega < -0.99f || 0.99f < cosOmega){
            ...
            f32 s0 = lray::sinf((1.0f-t) * omega) * oneOverSinOmega;
            k0 = _mm_set1_ps(s0);

            f32 s1 = lray::sinf(t * omega) * oneOverSinOmega;
            k1 = _mm_set1_ps(s1);
        }
